...
if(isset($_GET["questionId"]) != null){ // comçeo do código de editar pergunta
    $idQuestion = $_GET["questionId"];
}
$questionData = $question->select("*", ["idPergunta" => $idQuestion]);
...
<body>
    ...
    // Predefined values for the alternatives
    const predefinedValues = [ // definir dinamicamente para a quantidade de alternativas, e seus respectivos textos
        "Predefined Text 1",
        "Predefined Text 2",
        "Predefined Text 3",
        "Predefined Text 4",
    ...
    function populateOptions(qtdAlt) { //mudar o valor para a quantidade de alternativas registradas 
        ...
        for (let i = 1; i <= qtdAlt; i++) {
            ...
            // Set the predefined value if it exists
            if (predefinedValues[i - 1]) {
                textarea.value = predefinedValues[i - 1];
            }
...
if(isset($_POST["salvar"])){
    ...
    $quiz_has_question->create(["quiz_idquiz" => $quizId,
    "Pergunta_idPergunta" => $question->getPdo()->getConn()->lastInsertId()]);
}
?>
<br><a href='../user/menuUser/principal.php'> voltar ao menu principal</a>